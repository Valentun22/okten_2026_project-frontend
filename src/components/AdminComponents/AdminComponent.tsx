...
interface IAdminVenue {
    id: string; name: string; city?: string; avatarVenue?: string;
    isActive: boolean; isModerated: boolean;
    ratingAvg?: number; user?: { name?: string };
    categories?: { name: string }[];
}
interface IAdminUser {
    id: string; name?: string; email?: string; image?: string;
    isCritic?: boolean; role?: string[];
}
interface IComplaint {
    id: string; reason: string; type: string; status: string;
    created: string;
        ...
        <div>
            ...
            <div className={css.tableWrap}>
                <table className={css.table}>
                    <thead>
                    <tr>
                        <th>Фото</th><th>Назва</th><th>Місто</th><th>Власник</th>
                        <th>Модерація</th><th>Статус</th><th>Рейтинг</th><th>Дії</th>
                    </tr>
                    </thead>
                    <tbody>
                    {items.map(v => (
                            ...
                            <td>
                                <div className={css.actionBtns}>
                                    <button className={css.editBtn}
                                            onClick={() => navigate(`/venues/${v.id}/edit`)}>✏️</button>
    ...
    return (
        <div>
            ...
            <div className={css.tableWrap}>
                <table className={css.table}>
                    <thead>
                    <tr><th>Аватар</th><th>Ім'я</th><th>Email</th><th>Ролі</th><th>Дії</th></tr>
                    </thead>
                    <tbody>
                    {items.map(u => (
                        <tr key={u.id}>
...
const ComplaintsTab = () => {
    ...
    useEffect(() => { load(0); }, [filterStatus]); // eslint-disable-line
    ...
    return (
        <div>
            <div className={css.toolbar}>
                <select className={css.filter} value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setOffset(0); }}>
...
const TopTab = () => {
    ...
    const handleCatDragOver  = (e: React.DragEvent, idx: number) => { e.preventDefault(); dragOverCatIdx.current = idx; };
    const handleCatDrop = async () => {
        ...
        // PATCH /admin/top/categories/order — масив id у новому порядку
        await axiosInstance.patch(urls.admin.topCategoriesOrder,
            { ids: newOrder.map(c => c.id) }).catch(() => {});
    };

    // ── Venue order inside category ─────────────────────────────────────────
    ...
    const handleVenueDrop = async (catId: string) => {
        ...
        dragVenueInfo.current = null; dragOverVenueInfo.current = null;
        await axiosInstance.patch(urls.admin.topCategoryVenueOrder(catId),
            { ids: venues.map((v: any) => v.id) }).catch(() => {});
    };
            ...
            <div className={css.topCatList}>
                {cats.map((cat, catIdx) => (
                    <div key={cat.id} className={css.topCatCard}
                         ...
                         onDragEnd={() => { dragCatIdx.current = null; dragOverCatIdx.current = null; }}>
                        ...
                        {cat.venues && cat.venues.length > 0 && (
                            <div className={css.topVenueList}>
                                {cat.venues.map((v: any, vIdx: number) => (
                                    <div key={v.id} className={css.topVenueItem}
                                         draggable
                                         onDragStart={e => { e.stopPropagation(); handleVenueDragStart(cat.id, vIdx); }}
                                         onDragOver={e  => { e.preventDefault(); e.stopPropagation(); handleVenueDragOver(e, cat.id, vIdx); }}
                                         onDrop={e      => { e.stopPropagation(); handleVenueDrop(cat.id); }}>
                                        ...
                                        <button className={css.venueRemoveBtn}
                                                onClick={() => handleRemoveVenue(cat.id, v.id)}>✕</button>
                                    </div>
                                ))}
                            </div>
                        )}
