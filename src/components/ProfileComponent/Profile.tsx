...
interface IFavoriteVenue { id: string; name: string; avatarVenue?: string; city?: string; }
interface IMyComment {
    id: string; title: string; rating: number; created: string;
    venue?: { id: string; name: string };
}
interface IPyachokRow {
    id: string; date: string; time: string; purpose?: string;
    status: string; venue?: { name: string };
}
interface IFullUser {
    id: string; name?: string; email?: string; image?: string; bio?: string; isCritic?: boolean;
}
...
const Profile = () => {
    ...
    const handleSignOut = () => { dispatch(authActions.logout()); navigate('/'); };
                        ...
                        {tab === 'info' && (
                            <section className={css.section}>
                                ...
                                <div className={css.quickActions}>
                                    <button className={css.actionBtn} onClick={() => navigate('/searchVenue')}>🔍 Знайти заклад</button>
                                    <button className={css.actionBtn} onClick={() => navigate('/topVenues')}>🏆 Топ заклади</button>
                            ...
                            <section className={css.section}>
                                ...
                                {!loading && favorites.length === 0 && (
                                    <div className={css.emptyState}>
                                        ...
                                        <button className={css.actionBtn} onClick={() => navigate('/searchVenue')}>Знайти заклади</button>
                                    </div>
                                )}
                                <div className={css.listGrid}>
                                    {favorites.map(f => (
                                        <div key={f.id} className={css.listCard}>
                                            ...
                                            <button className={css.removeBtn} onClick={() => removeFav(f.id)} title="Видалити">✕</button>
                                        </div>
                                    ))}
                                </div>
                            </section>
                                            ...
                                            <div className={css.commentCardLeft}>
                                                <div className={css.commentStars}>
                                                    {Array.from({ length: 5 }).map((_, i) => (
                                                        <span key={i} className={i < c.rating ? css.starOn : css.starOff}>★</span>
                                                    ))}
                                                </div>
                                                ...
                                                <p className={css.commentDate}>
                                                    {new Date(c.created).toLocaleDateString('uk-UA', { day: 'numeric', month: 'long', year: 'numeric' })}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
